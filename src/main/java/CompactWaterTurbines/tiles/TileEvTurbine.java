...
public class TileEvTurbine extends TileEntity implements IWrenchable, IEnergyTile, IEnergySource, IInventory {

	public BasicSource energy;
	public Inventory inventory = new Inventory(1, "TileEvTurbine", 64);
	public static double euTick = 2048;
	public static int neededbuckets = 2048;
	private boolean initialized;
	public boolean hasWater = false;
	public static int euPerBucket = 2048000;
	int currentBuckets;

	public TileEvTurbine() {
		energy = new BasicSource(this, euPerBucket, 5);
	}
			...
			for (ForgeDirection fd : new ForgeDirection[] { ForgeDirection.UP, ForgeDirection.SOUTH,
				...
				if (!worldObj.isAirBlock(xCoord + fd.offsetX, yCoord + fd.offsetY, zCoord + fd.offsetZ)
					...
					boolean isWater = worldObj.getBlock(xCoord + fd.offsetX, yCoord + fd.offsetY,
							...
							&& worldObj.getBlock(xCoord + fd.getOpposite().offsetX, yCoord + fd.getOpposite().offsetY,
									zCoord + fd.getOpposite().offsetZ) == Blocks.water;
					if (isWater) {
						this.energy.addEnergy(euTick);
					}
					hasWater = isWater;
				}
		}
